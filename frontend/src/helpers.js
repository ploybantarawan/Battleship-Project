...
export const isShipCoordinate = (coordinate, ships) => {
  for (const { coordinates } of ships) {
    for( const coor of coordinates ){
      if ( checkIfSameCoordinate(coor, coordinate) ){
        return true
      }
    }
  }
...
export const getRandomUnshotCoordinate = (shotCoordinates) => {
  const nRow = 10
  const nCol = 10
  ...
  for(let r=0; r<nRow; ++r){
    for(let c=0; c<nCol; ++c){
      const coor = {row: r, column: c}
      if ( !checkIfLstIncludesCoordinate(shotCoordinates, coor) ){
        unShotCoors.push(coor)
      }
    }
  }

  return unShotCoors[Math.floor(Math.random()*unShotCoors.length)]}
